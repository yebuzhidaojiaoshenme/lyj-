...
current_table_type = None  # 当前处理的表格类型（Unmodified、Modified、Abbreviation、Screen 等）
...
for sheet_name, df in excel_file.items():
    ...
    while current_row < len(df):
        ...
        # 检测新的 Table 分隔符，使用正则表达式匹配 "Table X"（X 为number）
        ...
        if table_match:
            ...
            elif "Abbreviations" in row_text or sheet_name == "Sheet1":
                current_table_type = "Abbreviation"
            elif "Single Dose Screens" in row_text or sheet_name == "Sheet51":
                current_table_type = "Screen"
            # elif "Message Remaining" in row_text or "ST DEV" in row_text:  # 新增类型检测
            #     current_table_type = "MessageRemaining"
        ...
        if table_type == "Duplex_Table":
            ...
            save_sheet_name = "Sheet1" # 默认为sheet1
            ...
            if current_table and current_table_type == "MessageRemaining":
                table_num = int(current_table.replace('Table ', ''))
                save_sheet_name = f"Sheet{table_num}"
            else:
